...
from caf.ml.backlog.functions_to_be_processed.NorCom_caf_ml.larger_cafml_model.norcom_saved_model_funcs import (
    saved_model_func,
)
...
def process_prediction(params):
    ...
    if os.path.exists(training_dat_path):
        ...
        processed_data, index_columns_df = refined_data_processor_function(
            df=params.classified_build,
        ...
        training, test, validation = encode_and_sort(
            df=processed_data,
            target_column=params.target_column,
            output_folder=params.output_folder,
...
def process_saved(params):
    regression_method, hyperparameters, trained_data, transformations, data_analysis_data = (
        load_model_and_parameters(params.output_folder)
    )
    return saved_model_func(
        params,
        regression_method,
        hyperparameters,
        trained_data,
