...
def refined_data_processor_function(
    ...
    keep. The following array of functions_to_be_processed are taken directly from the
    ...
    if numerical_features is None:
        columns_to_keep = (
            index_columns + categorical_features + target_column_ + weight_column_
        )
    ...
    final_data = find_numeric_target_column(
        final_data, target_column=target_column, categorical_target=None
    )
    final_data = drop_rows(
        final_data,
...
def encode_and_sort(
    ...
    df = transform_target_column(
        df=df, target_column=target_column, binary_prediction=binary_prediction
    )
    ...
    training_df = data_encoded.loc[
        df.index.get_level_values("surveyyear") <= int(training_year)
    ]
...
def generate_stats_model(training_df, test_df, validation_df, target_column, output_folder):
    ...
    final_predictions = pd.DataFrame(
        {"predicted_target_column": pred_classes}, index=X_test.index
    )
...
def generate_sklearn_model(training_df, test_df, validation_df, target_column, output_folder):
    for df in [training_df, validation_df]:
        df[target_column] = df[target_column].apply(lambda x: x if x in [0, 1] else 2)
        df[target_column] = df[target_column].astype(int)
...
def generate_svm(training_df, test_df, validation_df, target_column, output_folder):
    for df in [training_df, validation_df]:
        df[target_column] = df[target_column].apply(lambda x: x if x in [0, 1] else 2)
        df[target_column] = df[target_column].astype(int)
    ...
    if os.path.exists(model_filename):
        ...
        grid_search = RandomizedSearchCV(
            model, param_distributions=param_grid, n_iter=50, cv=5, n_jobs=-1, verbose=1
        )
...
def caf_ml_logit(
    ...
    any_issues_present = refined_cafml_data_analysis(
        data=training_df,
        target_column=target_column,
        residuals=residuals,
        weight_column=weight_column,
    ...
    if any_issues_present is True or improve_data is not None:
        ...
        improved_data, transformations = modifying_data(
            data=training_df,
    ...
    test_df_transformed = apply_transformations(
    ...
    final_test_data = apply_feat_selection(
        trained_data=improved_data,
...
def generate_cafml_model(
    ...
    if os.path.exists(model_filename):
        ...
        training_df_modified = pd.read_csv(
            os.path.join(output_folder, "final_training_data.csv")
        )
        ...
        if os.path.exists(final_training_data_path):
            ...
            if any_issues_present is True or improve_data is not None:
                ...
                if skip_feature_selection is not None:
                    best_model = modified_hyper_optimisation(
                        model=model,
                        data=improved_data,
                        target_column=target_column,
                        output_folder=output_folder,
                ...
                training_df_modified = simple_feature_selection_test(
                    data=improved_data,
                    target_column=target_column,
                ...
                transformations = None
                training_df_modified = feat_selection_modified(
                    data=improved_data,
                    target_column=target_column,
                    cv=TimeSeriesSplit(n_splits=5),
                    regression_method=model,
    ...
    final_test_data = apply_feat_selection(
        trained_data=training_df_modified,
        test_data=test_df_transformed,
        output_folder=output_folder,
        target_column=target_column,
        index_columns=index_columns,
    ...
    simple_eval_model(
        training_df=training_df_modified,
        validation_df=validation_df,
        y_pred=y_pred,
        model=best_model,
        target_column=target_column,
...
def model_prep(training_df, target_column, output_folder, weight_column, model_to_use):
    ...
    models = {
        "gb": {
            "model": model_storage.gb,
            "filename": "gb_basic_cafml_modelfit.pkl",
            "params": param_grid_storage.gb_params,
        },
        "rf": {
            "model": model_storage.rf,
            "filename": "rf_basic_cafml_modelfit.pkl",
            "params": param_grid_storage.rf_params,
        },
        "dt": {
            "model": model_storage.dt,
            "filename": "dt_basic_cafml_modelfit.pkl",
            "params": param_grid_storage.dt_params,
        },
        "svm": {
            "model": model_storage.svm,
            "filename": "svm_basic_cafml_modelfit.pkl",
...
def refined_cafml_data_analysis(data, target_column, residuals, weight_column, threshold=5):
...
def simple_feature_selection_test(
    data, target_column, cv, regression_method, output_folder, weight_column, index_columns
):
    print("Feature selection beginning")
    final_train_filename = os.path.join(output_folder, "final_training_data.csv")
    if os.path.exists(final_train_filename):
    ...
    if mean_score < 0.6:
        print(
            "Initial feature selection attempt was inaccurate, trying \
              alternative method"
        )
        dataframe_final = feat_selection_modified(
            data=data,
            target_column=target_column,
            cv=cv,
...
def modified_hyper_optimisation(
    model,
    data,
    target_column,
    output_folder,
    ...
    if isinstance(model, GradientBoostingClassifier):
        ...
        if (
            model.get_params()["penalty"] == "l1"
            and model.get_params()["solver"] == "liblinear"
        ):
    ...
    if hasattr(best_model, "coef_"):
        ...
        if coefficients.ndim == 1:
            ...
            coeff_df = pd.DataFrame(
                coefficients.T, columns=[f"Class_{i}" for i in range(coefficients.shape[0])]
            )
        ...
        print(
            f"Coefficients saved to: {os.path.join(output_folder, 'final_model_coefficients.csv')}"
        )
    else:
...
def final_prediction(model, data, target_column, output_folder, validation, binary_prediction):
    ...
    final_predictions = pd.DataFrame(
        {"predicted_target_column": pred_classes}, index=data.index
    )
...
def feat_selection_modified(
    ...
    if cv_score < 0.5:
        ...
        data.to_csv(
            os.path.join(output_folder, "final_training_data.csv"), index=index_columns
        )
        return data
...
def simple_eval_model(training_df, validation_df, y_pred, model, target_column, output_folder):
...
def modifying_data(
    ...
    if applying_transformations is not None:
        ...
        if (
            len(features_to_transform) > 10
        ):  # this is an arbitrary threshold, investigate further
        ...
        mca_features = pd.DataFrame(
            mca_result,
            columns=[f"MCA{i + 1}" for i in range(n_components)],
            index=final_df.index,
        )  # changed from final_df to data
...
def apply_transformations(
    ...
    if weight_column in training_data_pre_feat_selection.columns:
        training_data_pre_feat_selection = training_data_pre_feat_selection.drop(
            columns=weight_column
        )

    if all(col in training_data_pre_feat_selection.columns for col in index_columns):
        training_data_pre_feat_selection = training_data_pre_feat_selection.set_index(
            index_columns
        )
    ...
    for transform_name, transform_obj in transformations:
        if transform_name == "interaction_poly_mca":
            transformed_data, _ = modifying_data(
