...
def main(params):
    ...
    if not os.path.exists(train_folder_name):
        imageprocessor = BuildImagesTT(output=main_output_folder,
                                       image_dict=None,
                                       image_location=params.image_path)
    ...
    if not os.path.exists(final_model_path):
        final_model_path = main_model_build(output=main_output_folder,
                                            hyperparameter_optimisation=params.hyperparameter_optimisation,
                                            path_to_code=params.path_to_code)

    prediction_results_dir, model = prediction(output=main_output_folder, final_model_path=final_model_path)
