...
from sklearn.model_selection import GridSearchCV, RandomizedSearchCV, ParameterGrid, TimeSeriesSplit, KFold
    ...
    for name, member in Models.__members__.items():
        if isinstance(member.value, tuple):
            ...
            if expected_class == model_class:
                ...
                if all(instance_params.get(key) == value for key, value in combined_params.items()):
                    return name
        else:
...
def select_param(data,
    ...
    cv = TimeSeriesSplit(n_splits=5) if multiple_year_prediction is not None else KFold(n_splits=5)
    ...
    def perform_random_search(model_instance,
                              param_grid,
                              cv,
                              scoring):
    ...
    if categorical_data is not None:
        ...
        if isinstance(model_instance, (RandomForestClassifier, ExtraTreesClassifier, DecisionTreeClassifier)):
            if isinstance(model_instance, DecisionTreeClassifier):
                model_instance.set_params(max_depth=10)
                return perform_random_search(model_instance, param_grid, cv=cv, scoring="accuracy")
            else:
    ...
    if categorical_data is None:
        if isinstance(model_instance, (RandomForestRegressor, ExtraTreesRegressor, DecisionTreeRegressor)):
