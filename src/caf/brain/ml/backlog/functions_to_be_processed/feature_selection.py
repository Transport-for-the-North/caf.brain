...
def feature_selection_cv(
    ...
    cv = (
        ...
        else get_cv_class(cv_method, splits=splits, repeats=repeats)
    )

    if categorical_data is not None:
        for train_index, test_index in tqdm(
            cv.split(x, y), total=cv.get_n_splits(x, y), desc="Feature selection progress"
        ):
            ...
            score_rfe = cross_val_score(
                model, X_train_rfe, y_train, scoring="accuracy", cv=cv
            ).mean()
            ...
            score_f_regressor = cross_val_score(
                model, X_selected_mi, y_train, scoring="accuracy", cv=cv
            ).mean()
        ...
        for train_index, test_index in tqdm(
            ...
            if isinstance(model, (KNeighborsRegressor, BaggingRegressor, SVR)):
                ...
                score_model = cross_val_score(
                    model, X_train_model, y_train, scoring="neg_mean_squared_error", cv=cv
                ).mean()
                ...
                selected_f_regressor = SelectKBest(f_regression, k="all").fit(
                    X_train_scaled, y_train
                )
                ...
                score_rfe = cross_val_score(
                    model, X_train_rfe, y_train, scoring="neg_mean_squared_error", cv=cv
                ).mean()
                if score_rfe > best_score_rfe:
                    best_score_rfe = score_rfe
