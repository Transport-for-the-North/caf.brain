...
def main_feature_selection(
    ...
    if train is None and test is None:
        LOG.info(
            "Train and test are none so they are being generated "
            "by the main_input_data function"
        )
    ...
    if train is None or test is None:
        raise ValueError(
            "Train and test data must not be None after loading. \n"
            "Either provide train and test directly or provide a path \n"
            "to external data that will be used as train and test."
        )
    ...
    if modelling.intensive_feature_selection and initialised_model is None:
        LOG.warning(
            ...
            " can be done using the main_model_selection function."
        ...
        train_final.to_csv(
            output / "final_train.csv", index=not isinstance(train_final.index, pd.RangeIndex)
        )
        test_final.to_csv(
            output / "final_test.csv", index=not isinstance(test_final.index, pd.RangeIndex)
        )
