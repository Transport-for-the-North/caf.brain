...
# Local Imports
from caf.brain.object_detection._functions.model_building.build_model_functions import (
    _baseline_model,
    _final_model,
    _model_comparison,
)
from caf.brain.object_detection._functions.model_building.hyper_optim_functions import (
    EarlyStopper,
...
def _main_hyperparameter_optimisation(
    basemodel: YOLO,
    config: str | Path,
...
def main_model_build(output: Path, hyperparameter_optimisation: str | None = "base") -> None:
    ...
    if final_model_path.exists():
        ...
        _main_hyperparameter_optimisation(
            basemodel=model,
            config=config_path,
