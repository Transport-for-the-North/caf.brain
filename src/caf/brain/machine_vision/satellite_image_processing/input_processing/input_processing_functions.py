...
from caf.brain.machine_vision.satellite_image_processing.html_processing.html_processing_main import main_process_html
from caf.brain.machine_vision.satellite_image_processing.image_processing.image_processing_functions import euclidean_distance
import logging
LOG = logging.getLogger(__name__)


def process_coordinates(geo_df: gpd.geodataframe,
                        df: pd.DataFrame,
                        image_folder: Path,
    ...
    if image_folder is None:
        raise ValueError("Ensure that a path to an image folder is provided \
    ...
    if geo_df is not None:
        ...
        df = df.rename(columns={x_coordinate: 'coordinates_easting', y_coordinate: 'coordinates_northing'})

    else:
        raise ValueError("Must provide one of geo_df or df. If geo_df then it \
                         should contain a geometry column. If df then the csv should \
                         contain two coordinates columns.")

    coordinate_data, unique_box_boundaries = euclidean_distance(df_a=final_html,
                                                                df_b=df)
