...
from caf.brain.ml._ml import evaluate_data, tidy_data, transform_data, feature_selection
...
def main():
    ...
    df = tidy_data(data_path=Path(path),
    ...
    transform_df = transform_data(data=df,
                                  output_path=Path(output),
                                  categorical_features=['soc', 'hh_child'],
                                  numerical_features=['trips'],
                                  target='numcarvan',
    ...
    feat_select_train = feature_selection(data=train,
                                       output_path=Path(output),
                                       categorical_features=['soc', 'hh_child'],
                                       numerical_features=['trips'],
                                       target='numcarvan',
    ...
    model = algorithm_evaluation(data=feat_select_train,
                                output_path=Path(output),
                                target='numcarvan',
    ...
    best_model = hparam_optim(model_choice=Models.LOGIT_REGRESSION_ELASTICNET,
