...
from sklearn.feature_selection import SelectFromModel, SelectKBest, RFE, mutual_info_regression, \
    f_regression, RFECV, mutual_info_classif
...
def feature_selection_test(train_transformed,
                           cv,
                           model,
                           weight_column,
                           target_column,
    ...
    if binary_prediction is not None:
        for train_index, test_index in tqdm(cv.split(x), total=cv.get_n_splits(x), desc="Feature selection progress"):
            ...
            score_rfe = cross_val_score(model, X_train_rfe, y_train, scoring=scoring, cv=cv).mean()
            ...
            score_f_regressor = cross_val_score(model, X_selected_mi, y_train, scoring=scoring, cv=cv).mean()
        ...
        final_data = filter_data(original_data=train_transformed,
        ...
        for train_index, test_index in tqdm(cv.split(x), total=cv.get_n_splits(x), desc="Feature selection progress"):
            ...
            if isinstance(model, (KNeighborsRegressor, BaggingRegressor, SVR)):
                ...
                selector_mutual_info = SelectKBest(mutual_info_regression, k=5).fit(X_train, y_train)
                ...
                score_rfe = cross_val_score(model, X_train_rfe, y_train, scoring=scoring,
                                            cv=cv).mean()
                if score_rfe > best_score_rfe:
                    best_score_rfe = score_rfe
