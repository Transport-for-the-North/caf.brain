...
def select_model(
    x: pd.DataFrame, y: pd.DataFrame, models_to_test: list[Models], output_folder
):
    ...
    for model_enum in models_to_test:
        ...
        scores_mse = -cross_val_score(
            model_instance, x, y, cv=5, scoring="neg_mean_squared_error", n_jobs=-1
        )
...
def eval_model(
    ...
    if categorical_target is not None:
        ...
        precision, recall, fscore, _ = precision_recall_fscore_support(
            y_truth, model_predicted_data, average="weighted"
        )
        ...
        metrics_df.to_csv(
            os.path.join(output_folder, "model_evaluation_metrics.csv"), index=False
        )
        ...
        stats_df = simple_logistic_regression_stats(
            model,
            data_used_to_predict,
            output_folder,
            target_column,
    ...
    if (model_predicted_data < 0).any():
        ...
        negative_predictions_df = pd.DataFrame(
            {
                "Prediction": model_predicted_data[model_predicted_data < 0],
                "True_Label": y_truth[model_predicted_data < 0],
            }
        )
        negative_predictions_df.to_csv(
            os.path.join(output_folder, "negative_predictions.csv"), index=False
        )
    else:
    ...
    if isinstance(model, (LinearRegression, Ridge, Lasso, ElasticNet)):
        calculate_non_categorical_model_stats(
            model, data_used_to_predict, target_column, output_folder
        )
...
def plot_regression_diagnostics(y_truth, model_predicted_data, model, output_folder):
    ...
    plt.text(
        0.05,
        0.95,
        f'R-squared: {metrics_dict["R-squared"]:.2f}',
...
def calculate_non_categorical_model_stats(
    model, data_used_to_predict, target_column, output_folder
):
    ...
    results_df.to_csv(
        os.path.join(output_folder, "non_categorical_model_stats.csv"), index=False
    )

    return results_df


def simple_logistic_regression_stats(
    model, data_used_to_predict, output_folder, target_column, model_predicted_data, y_proba
):
    ...
    results_df.to_csv(
        os.path.join(output_folder, "logistic_regression_stats.csv"), index=False
    )
    return results_df
