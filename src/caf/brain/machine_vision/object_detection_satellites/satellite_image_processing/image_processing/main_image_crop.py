...
from caf.brain.machine_vision.object_detection_satellites.satellite_image_processing.image_processing.image_processing_functions import check_raster_file
...
from caf.brain.machine_vision.object_detection_satellites.satellite_image_processing.image_processing.expand_image_functions import (
    ...
    create_new_image, find_surrounding_images)


def image_crop_main(user_coordinate_data: pd.DataFrame,
                    output: Path,
                    image_folder: Path,
                    satellite_metadata: pd.DataFrame) -> None:
    """
    ...
    with tqdm(total=None) as pbar:
        for path in user_coordinate_data['paths']:
            ...
            output_filename_extended = os.path.join(output_dir, f"{file_name}_cropped_extended.jpg")

            if os.path.exists(output_filename) or os.path.exists(output_filename_extended):
                LOG.info("Skipping image processing as final cropped image already exists in \
                          output directory.")
                continue
                ...
                with rasterio.open(path) as img:
                    ...
                    if list_of_needed_tiles:
                        ...
                        if not image_paths_to_concat:
                            LOG.warning("Required satellite images to extend centre image for crop \
                                         does not exist")
                        ...
                        if actual_width <= 0 or actual_height <= 0:
                            ...
                            LOG.warning("Adjusted window: col_start=%s, col_end=%s, row_start=%s, row_end=%s", col_start, col_end, row_start, row_end)
                            LOG.warning("Image dimensions: width=, height=%s", img.width, img.height)
