...
def norcom_run_functions(params, training, test, validation):
    ...
    valid_models = (
        [model.value for model in Models] + Default_regression_methods + Models_List_
    )
    ...
    model = (
        model_type[0].get_model_instance()
        if isinstance(model_type[0], Models)
        else model_type[0]
    )
    print(f"Model to be used: {model}")

    dat, transformations = pre_forecast_data_analysis_modified(
        data=training,
    ...
    hyperparameters, best_model = modified_hyper_optimisation(
        regression_method=model,
        data=dat,
        target_column=params.target_column,
        output_folder=params.output_folder,
    ...
    test = apply_transformations_norcom(
        predict_data=test,
        transformations=transformations,
        target_column=params.target_column,
    ...
    final_predict_data = align_dataframes(
        df1=dat, df2=test, output_folder=params.output_folder
    )

    predictions, y_proba = predict_refined(
        trained_data=dat,
...
def feat_selec_feat_importance(
    output_folder,
    dat,
    df,
    model,
    ...
    if hasattr(model, "feature_importances_") or hasattr(model, "coef_"):
        ...
        feature_importance = pd.DataFrame(
            {"feature": features_df.columns, "importance": importance_values}
        ).sort_values("importance", ascending=False)

        feature_importance.to_csv(
            os.path.join(output_folder, "feature_importances.csv"), index=False
        )
        ...
        for top_n in tqdm(
            ...
            desc="Evaluating features with feature importance",
        ):
        ...
        print(
            "Model does not have feature importances. Select model that does. \
...
def modified_hyper_optimisation(regression_method, data, target_column, output_folder):
    ...
    def _grid_search(model_instance, param_grid, cv, scoring):
        grid_search = RandomizedSearchCV(
            model_instance, param_grid, cv=cv, scoring=scoring, verbose=2, n_jobs=-1
        )
    ...
    coef_df.to_csv(
        os.path.join(output_folder, "coefficients_from_trained_model.csv"), index=True
    )

    return best_params, best_model


def apply_transformations_norcom(
    predict_data,
    transformations,
    target_column,
    numerical_features,
    ...
    data = convert_to_dataframe(
        transformed_data,
        columns=training_data_pre_feat_selection.columns,
...
def create_complex_hybrid_sample(df, target_column, output_folder):
    ...
    strat_split = StratifiedShuffleSplit(
        n_splits=1, test_size=sample_fraction, random_state=42
    )
...
def pre_forecast_data_analysis_modified(
    data,
    regression_method,
    target_column,
    threshold,
