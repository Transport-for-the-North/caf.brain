...
def moderate_optimisation(config, output_dir, model, path_to_code):
    ...
    if os.path.exists(best_hyp_path):
        ...
        study.optimize(
            lambda trial: optuna_objective_func(trial, config, output_dir, model, path_to_code),
            n_trials=100,
    ...
    optimisation_results = {'best_hyperparameters': study.best_params,
                            'performance_metrics': performance_metrics,
                            'trial_details': {'trial_id': study.best_trial.number,
...
#     runs_path = os.path.join(path_to_code, 'machine_vision', 'yolo_object_detection', 'runs')
#     if os.path.exists(runs_path):
#         LOG.info(f"Removing YOLO runs directory: {runs_path}")
#         os.remove(runs_path)


def optuna_objective_func(trial, config, output, model, path_to_code):
