...
from caf.brain.machine_vision.noham_processing.noham_inputs import NoHAMInputs


def noham_data(geo_path: Path, noham_path: Path, output: Path) -> pd.DataFrame:
    ...
    df.loc[df["RDCLASS"].isin(["Minor Road", "MINOR ROAD", "Minor", "MINOR"]), "RDCLASS"] = (
        "minor_road"
    )
...
def main_(parmas: NoHAMInputs):
    noham_dat = noham_data(
        geo_path=parmas.noham_shp_path,
...
if __name__ == "__main__":
    params = NoHAMInputs(
        output_path=Path(
            r"E:\2025 work streams\caf.brAIn\machine vision\MVP work\input_image_processing"
        ),
        noham_db_path=Path(
            r"E:\2025 work streams\caf.brAIn\machine vision\MVP work\input_image_processing\input\Base_2018_shapefiles\NoHAM_Base.DBF"
        ),
        noham_shp_path=Path(
            r"E:\2025 work streams\caf.brAIn\machine vision\MVP work\input_image_processing\input\Base_2018_shapefiles\NoHAM_Base_node.shp"
        ),
    )
    main_(params)
