...
def train_model(X_train,
    ...
    evaluation_results = evaluate_model(model, X_train, y_train, X_predict, y_validate, output_folder)
    ...
    prediction_df = pd.DataFrame({target_column: predict_predictions.flatten()},
...
def train_improved_model(X_train, y_train, X_predict, y_validate, output_folder, index_col,
                         target_column, predict_data, epochs=100, batch_size=32):
    ...
    if os.path.exists(model_path):
        ...
        X_train_split, X_val_split = np.split(X_train,
                                              [int((1 - validation_split) * len(X_train))])
        y_train_split, y_val_split = np.split(y_train,
                                              [int((1 - validation_split) * len(y_train))])
