...
def ray_optuna_objective_func(model, config_path, config):
    ...
    results = {"metrics/mAP50-95(B)": metrics.box.map,
...
def exhaustive_optimisation(model, config_path, output_dir):
    search_space = {"lr0": tune.loguniform(1e-5, 1e-1),
    ...
    optuna_search = OptunaSearch(metric="metrics/mAP50-95(B)",
                                 mode="max")
    ...
    analysis = tune.run(tune.with_parameters(ray_optuna_objective_func,
    ...
    optimisation_results = {'best_hyperparameters': best_config,
                            'performance_metrics': {'mAP50': best_result.get('metrics/mAP50(B)', None),
                            ...
                            'trial_details': {'trial_id': best_trial.trial_id,
