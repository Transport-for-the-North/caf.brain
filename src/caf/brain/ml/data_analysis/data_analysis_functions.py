...
def pre_forecast_data_analysis(
    ...
    is_classification = isinstance(
        ...
        is_statsmodel
        and ("Logit" in str(model.__class__) or "MNLogit" in str(model.__class__))
    )

    is_linear_model = isinstance(
        model, (LinearRegression, Ridge, Lasso, ElasticNet, LogisticRegression)
    ) or (
        is_statsmodel
        and any(name in str(model.__class__) for name in ["OLS", "GLM", "Logit", "MNLogit"])
    )
    ...
    if any(issues.values()) and full_transformations:
        LOG.warning(
            "Data issue present, corrective transformations applied to numerical features"
        )
