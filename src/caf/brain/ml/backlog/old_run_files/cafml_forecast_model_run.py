...
from caf.ml.backlog.functions_to_be_processed import eval_model
from caf.ml.backlog.functions_to_be_processed import norcom_run_functions
from caf.ml.backlog.old_inputs import CarAccessInputs

warnings.filterwarnings("ignore")

from caf.ml.backlog.functions_to_be_processed import DataProcessor
from caf.ml.backlog.functions_to_be_processed import feature_selection_cv, filter_data
from caf.ml.backlog.functions_to_be_processed import select_param
...
def saved_model_functions(params):
    regression_method, hyperparameters, trained_data, transformations = (
        load_model_and_parameters(params.output_folder)
    )
    ...
    predict_data_post_transformation = apply_transformations(
    ...
    final_predict_data = align_dataframes(
        df1=trained_data,
        df2=predict_data_post_transformation,
        output_folder=params.output_folder,
    )
...
def process_prediction(params):
    ...
    preprocessed_df, transformations_ = process_data_pipeline(
        df=processed_data,
        numerical_features=params.numerical_features,
        categorical_features=params.categorical_features,
        target_column=params.target_column,
    ...
    if params.skip_feature_selection:
        ...
        return process_skip_data_analysis(
            params,
            preprocessed_df,
            transformations_,
            model,
        ...
        return process_skip_hyperparameter_optimisation(
            params,
            preprocessed_df,
            transformations_,
            model,
...
def process_skip_feature_selection(params, processed_data, transformations_, model):
    df_final_to_model, transformations = pre_forecast_data_analysis(
        data=processed_data,
    ...
    hyperparameters = select_param(
        data=df_final_to_model,
        target_column=params.target_column,
        model=model,
        categorical_data=params.categorical_data,
        multiple_year_prediction=params.multiple_year_prediction,
...
def process_skip_data_analysis(
    params,
    preprocessed_df,
    transformations_,
    model,
...
def process_skip_hyperparameter_optimisation(
    params,
    preprocessed_df,
    transformations_,
    model,
...
def process_regular_model(
    ...
    if params.norcom_run is not None:
        return norcom_run_functions(
            params, preprocessed_df=preprocessed_df, transformations_=transformations_
        )
    ...
    final_predict_data = align_dataframes(
        df1=df_final_to_model, df2=dat, output_folder=params.output_folder
    )
